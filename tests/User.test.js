...
describe("User", () => {
  ...
  it("should not work if the user doesn't exist anymore", (done) => {
    request(GraphQLServer)
      ...
      .then((res) => {
        const parsedRes = JSON.parse(res.text)
        expect(parsedRes.errors).toBeTruthy()
        expect(parsedRes.errors[0].message).toBe("User doesn't exist. Use `SignupUser` to create one")
        done()
      })
  })
  ...
  it("should be able to activate 2F authentication", async (done) => {
    ...
    const parsedRes = JSON.parse(res.text)
    expect(parsedRes.errors).toBeUndefined()
    expect(parsedRes.data.UpgradeTo2FactorAuthentication.secret).toBeTruthy()
    self.twoFactorSecret = parsedRes.data.UpgradeTo2FactorAuthentication.secret

    expect(parsedRes.data.UpgradeTo2FactorAuthentication.qrCode).toBeDefined()
    done()
  })
  ...
  it("should keep the same 2F authentication secret for the following upgrades", async (done) => {
    ...
    const parsedRes = JSON.parse(res.text)
    expect(parsedRes.errors).toBeUndefined()
    expect(parsedRes.data.UpgradeTo2FactorAuthentication.secret).toBe(self.twoFactorSecret)
    ...
    const res = await request(GraphQLServer)
      ...
      .send({
        ...
        variables: {
          email: "gianni@pinotto.it",
          password: "password",
          twoFactorCode: OTP.googleAuthenticator.gen(self.twoFactorSecret),
        },
      })
    const parsedRes = JSON.parse(res.text)
    expect(parsedRes.errors).toBeUndefined()
